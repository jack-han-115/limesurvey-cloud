            ...
            if (isset($fnames[$i]['type']) && $fnames[$i]['type'] == "|")
            {
                ...
                if (isset($phparray[$index]))
                {
                    ...
                    <?php }
                        ...
                        <td>
                        <?php 
                        $sFileName = Yii::app()->getConfig('uploaddir') . "/surveys/" . $iSurveyID . "/files/" . $phparray[$index]['filename'];
                        if (file_exists($sFileName))
                        {?>
                            <a href='#' onclick=" <?php echo convertGETtoPOST('?action=browse&amp;subaction=all&amp;downloadindividualfile=' . $phparray[$index][$metadata] . '&amp;fieldname=' . $fnames[$i][0] . '&amp;id=' . $dtrow['id'] . '&amp;sid=' . $surveyid); ?>" ><?php echo rawurldecode($phparray[$index][$metadata]); ?></a></td>                            
                        <?php } else 
                        {?>
                            <?php echo rawurldecode($phparray[$index][$metadata]); ?> (missing)</td>                            
                         <?php }?>
                        <?php }
                        else
                        { ?>
