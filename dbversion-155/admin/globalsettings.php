    ...
    function globalsettingssave()
    {
        ...
        if (isset($action) && $action == "globalsettingssave")
        {
            if($_SESSION['USER_RIGHT_CONFIGURATOR'] == 1)
            {
                ...
                if ($_POST['emailmethod']!='mail' && $_POST['emailmethod']!='smtp')
                {
                    $_POST['emailmethod']='mail';
                }
                ...
                $emailsmtphost=returnglobal('emailsmtphost');
                if (strpos($emailsmtphost,'localh')!==false
                    || strpos($emailsmtphost,'127.0.0')!==false
                    || strpos($emailsmtphost,'78.46.85.')!==false
                    || strpos($emailsmtphost,'78.46.72.')!==false
                    || strpos($emailsmtphost,'lime')!==false)  $emailsmtphost='';
                setGlobalSetting('emailsmtphost',strip_tags($emailsmtphost));
                ...
                if (strpos($_SERVER['SERVER_NAME'],'limequery.com')===false)
                {
                    $_POST['force_ssl']='off';
                }
    ...
    function globalsettingsdisplay()
    {
        ...
        if (isset($action) && $action == "globalsettings")
        {
            if($_SESSION['USER_RIGHT_CONFIGURATOR'] == 1)
            {
                ...
                $editsurvey .= "<br /></p><div class='settingcaption'>".$clang->gT("Updates")."</div><p>\n";
                $updateavailable=0;
                ...
                $editsurvey .= ">".$clang->gT("Off")."</option>\n"
                . "</select>\n</li>\n"
                . "<li style='display:none;'><label for='sessionlifetime'>".$clang->gT("Session lifetime (seconds):")."</label>\n"
                ...
                $editsurvey .= ">".$clang->gT("LimeService")."</option>\n"
                . "\t\t\t<option value='smtp'";
                ...
                $editsurvey .= ">".$clang->gT("Off").$clang->gT("/LimeService")."</option>\n"
                . "\t\t\t<option value='IMAP'";
                ...
                $warning_force_ssl = $warning_force_ssl=$clang->gT('Attention: If you are enabling/forcing SSL then 0.5 responses are counted additionally if a survey is started and using SSL encryption (a complete responses will cost then 1.5 LimeService responses).');
                ...
                if ($statdatabaseport!="default") {$dbport="$statdatabaselocation:$statdatabaseport";}
                else {$dbport=$statdatabaselocation;}
                if ($statconnect->Connect($statdbport, $statdatabaseuser, $statdatabasepass, $statdatabasename))
                {
                    $usresult=@$statconnect->Execute("select upgradedbversion from pageviews where subdomain='$subdomain' and rootdomain='$domain'");
                    $usrow = $usresult->FetchRow();
                    if (intval($usrow['upgradedbversion'])<$destinationversion)
                    {
                        if ($subaction=='scheduleupgrade' )
                        {
                            $usquery = "SELECT stg_value FROM ".db_table_name("settings_global")." where stg_name='ScheduledDBVersion'";
                            $usresult = db_execute_assoc($usquery,'',true); //checked
                            if ($usresult->RecordCount()==0) // Never upgraded
                            {
                                // Schedule for upgrade
                                $statconnect->Execute("Update pageviews set upgradedbversion=$destinationversion where subdomain='$subdomain' and rootdomain='$domain'");
                                unset($statconnect);
                                $editsurvey.= '<div style="width:600px;text-align: left;padding-top:50px; margin:0 auto;">'
                                .'<span style="font-weight: bold; font-size: 14pt;">Upgrade to '.$upgradeversion.' is now scheduled!</span><br /><br />'
                                .'<b>Your LimeService installation was scheduled for upgrade.</b> Upgrade cycles run every full hour. The current server time is '.date('g:i a').'. ';
                                $editsurvey.= 'This action cannot be undone. When the update process has finished you will receive an email.<p /></div>';
                        ...
                        else //explain the upgrade
                        {
                            ...
                            $editsurvey.= "<form action='$scriptname?action=globalsettings' method='post' onsubmit='return confirm(\"Are you sure you want to upgrade to version $upgradeversion? We strongly recommend that you only upgrade if you do not have active surveys!\");' >"
                            ."<p><input type='hidden' id='subaction' name='subaction' value='scheduleupgrade' /><input type='submit' value='Upgrade to $upgradeversion!'/>\n";
                            $editsurvey.= "</form><p>";
                        }
                    }
                    else
                    {
                        $editsurvey.= '<div style="width:600px;text-align: left;padding-top:50px; margin:0 auto;">'
                        .'<span style="font-weight: bold; font-size: 14pt;">Upgrade to '.$upgradeversion.' is in progress!</span><br /><br />'
                        .'Your LimeService installation is already scheduled for upgrade. Upgrade cycles run on every full hour and can take up to 15 minutes - please be patient. The current server time is '.date('g:i T').'.';
                        $editsurvey.= 'This action cannot be undone. <br /><b>Please be patient. When the upgrade has finished you will receive an automatic email.</b><p /></div>';
                    }
                }
                unset($statconnect);
