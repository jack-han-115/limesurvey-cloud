    ...
    if (bHasSurveyPermission($surveyid, 'responses','read') || bHasSurveyPermission($surveyid, 'responses','create')  || bHasSurveyPermission($surveyid, 'responses','update'))
    {
        ...
        if ($subaction == "insert" && bHasSurveyPermission($surveyid,'responses','create'))
        {
            ...
            else
            {
                ...
                if (isset($_POST['closerecord'])) {$hit='1';} else {$hit='0.5';}
                if (@$statconnect->Connect($statdbport, $statdatabaseuser, $statdatabasepass, $statdatabasename))
                {
                    $statconnect->Execute("Update responses set hits=hits+$hit, modified=NOW() where subdomain='$subdomain' and rootdomain='$domain' and hitperiod='".date('Y-m-d H:00:00')."'");
                    if ($statconnect->Affected_Rows()==0)
                    {
                        $statconnect->Execute("insert into responses (hits,subdomain,rootdomain,hitperiod, created) values ($hit,'$subdomain','$domain','".date('Y-m-d H:00:00')."', NOW())");
                    }
                }
                unset($statconnect);
        ...
        elseif ($subaction == "update"  && bHasSurveyPermission($surveyid,'responses','update'))
        {
            ...
            //===============Begin LimeService Mod
            if (isset($_POST['submitdate']) && $_POST['submitdate'] && $_POST['submitdate'] != "N"  && ($_POST['oldsubmitdate'] == "" || $_POST['oldsubmitdate'] == "N")) {
                $statconnect=&ADONewConnection($statdatabasetype);
                $domain=$_SERVER['SERVER_NAME'];
                $subdomain=substr($domain,0,strpos($domain,'.'));
                $domain=substr($domain,strpos($domain,'.')+1);
                ...
                if (@$statconnect->Connect($statdbport, $statdatabaseuser, $statdatabasepass, $statdatabasename))
                {
                    $statconnect->Execute("Update responses set hits=hits+0.5, modified=NOW() where subdomain='$subdomain' and rootdomain='$domain' and hitperiod='".date('Y-m-d H:00:00')."'");
                    if ($statconnect->Affected_Rows()==0)
                    {
                        $statconnect->Execute("insert into responses (hits,subdomain,rootdomain,hitperiod, created) values (0.5,'$subdomain','$domain','".date('Y-m-d H:00:00')."', NOW())");
                    }
                }
                unset($statconnect);
            }
