        ...
        */
        private function _UpdateValuesInDatabase($updatedValues, $finished=false,$setSubmitDate=false)
        {
            ...
            if (count($updatedValues) > 0 || $finished)
            {
                ...
                if (!isset($_SESSION['limeservice_start']) && $this->surveyOptions['active']=='Y' && ($_SESSION['step']==1 || ($_SESSION['totalsteps']==1 && $_SESSION['step']==2)))
                {
                    global $statdatabasetype, $statdatabaseport, $statdatabaseuser, $statdatabasepass, $statdatabasename, $statdatabaselocation;
                    $statconnect=&ADONewConnection($statdatabasetype);
                    $domain=$_SERVER['SERVER_NAME'];
                    $subdomain=substr($domain,0,strpos($domain,'.'));
                    $domain=substr($domain,strpos($domain,'.')+1);
                    if ($statdatabaseport!="default") {$statdbport="$statdatabaselocation:$statdatabaseport";}
                    else {$statdbport=$statdatabaselocation;}

                    $iResponsesToAdd=0.5;
                    if ($_SERVER['HTTPS'] != '') $iResponsesToAdd=$iResponsesToAdd+0.5;
                    if (@$statconnect->Connect($statdbport, $statdatabaseuser, $statdatabasepass, $statdatabasename))
                    {
                        $statconnect->Execute("Update responses set hits=hits+{$iResponsesToAdd}, modified=NOW() where subdomain='$subdomain' and rootdomain='$domain' and hitperiod='".date('Y-m-d H:00:00')."'");
                        if ($statconnect->Affected_Rows()==0)
                        {
                            $statconnect->Execute("insert into responses (hits,subdomain,rootdomain,hitperiod, created) values ({$iResponsesToAdd},'$subdomain','$domain','".date('Y-m-d H:00:00')."', NOW())");
                        }
                    }
                    unset($statconnect);
                    $_SESSION['limeservice_start']=true;
                }
                //===============End LimeService Mod
