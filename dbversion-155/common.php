...
require_once ($rootdir.'/classes/phpmailer/class.smtp.php');
...
SSL_mode(getGlobalSetting('force_ssl'));  
...
If (!$dbexistsbutempty && $sourcefrom=='admin')
{
    ...
    if (is_dir($homedir."/install") && $debug<2)
    {
     ...
     { 
        $strinsert=",lastaccess='".date('Y-m-d H:i:s')."'";
        $statconnect->Execute("Update pageviews set pageviews_admin=pageviews_admin+1$strinsert,lastwarning=null where subdomain='$subdomain' and rootdomain='$domain'");
        if ($statconnect->Affected_Rows()==0)
        {
          $statconnect->Execute("insert into pageviews (pageviews_admin,subdomain,rootdomain,lastaccess) values (1,'$subdomain','$domain','".date('Y-m-d H:i:s')."')");
        } 
     }
     unset($statconnect);
     // ========================  End LimeService Mod
 ...
 else
 {
     ...
     if (@$statconnect->Connect($statdbport, $statdatabaseuser, $statdatabasepass, $statdatabasename))
     { 
        $statconnect->Execute("Update pageviews set pageviews_client=pageviews_client+1 where subdomain='$subdomain' and rootdomain='$domain'");
        if ($statconnect->Affected_Rows()==0)
        {
          $statconnect->Execute("insert into pageviews (pageviews_client,subdomain,rootdomain) values (1,'$subdomain','$domain')");
        } 
     }
     unset($statconnect);
     // ========================  End LimeService Mod
