            ...
            if ($emcount > 0) {
                ...
                foreach ($emresult as $emrow) {
                    ...
                    if ($iEmailLocked == 1 && Yii::app()->getConfig('emailmethod') != 'smtp') {
                        $success = false;
                        $mail->ErrorInfo = gT('You are currently banned from sending emails using the LimeSurvey email servers. Please configure your global settings to use your own SMTP server, instead. If you have any questions regarding this ban, please contact support@limesurvey.org.', 'unescaped');
                    } elseif ($bSpamLinks) {
                        $success = false;
                        $mail->ErrorInfo = gT('Using the FREE & BASIC plan only links to your survey are allowed.', 'unescaped');
                    } elseif ($bnoSurveyLink) {
                        $success = false;
                        $mail->ErrorInfo = gT('Your email must contain an invitation link to the survey.', 'unescaped');
                    } else {
